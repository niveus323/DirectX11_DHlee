...
class Exercise_Chapter9_2 : public D3DApp
{
public:
	Exercise_Chapter9_2(HINSTANCE hInstance);
	~Exercise_Chapter9_2();
	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise_Chapter9_2 theApp(hInstance);
...
Exercise_Chapter9_2::Exercise_Chapter9_2(HINSTANCE hInstance)
...
{
	mMainWndCaption = TEXT("연습문제 9장 2~3번");
	mEnable4xMsaa = false;
...
Exercise_Chapter9_2::~Exercise_Chapter9_2()
{
...
void Exercise_Chapter9_2::OnResize()
{
	D3DApp::OnResize();
...
void Exercise_Chapter9_2::UpdateScene(float dt)
{
	...
	if (GetAsyncKeyState('1') & 0x8000)
	{
		...
		blendDesc.IndependentBlendEnable = false;
		blendDesc.RenderTarget[0].BlendEnable = true;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA; //원본 혼합 계수
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;//대상 혼합 계수
		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
		blendDesc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;//알파 성분 혼합의 원본 혼합계수
		blendDesc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;//알파 성분 혼합의 대상 혼합계수
		blendDesc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
		blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;
...
void Exercise_Chapter9_2::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		//연습문제 3번 - 지형보다 물을 먼저 그릴 경우 물에 적용되는 혼합 상태의 대상 픽셀이 지형이 아닌 배경으로 지정된다
		//
		// Draw the waves.
		//
...
void Exercise_Chapter9_2::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Exercise_Chapter9_2::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Exercise_Chapter9_2::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Exercise_Chapter9_2::GetHillHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Exercise_Chapter9_2::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void Exercise_Chapter9_2::BuildLandGeometryBuffers()
{
...
void Exercise_Chapter9_2::BuildWaveGeometryBuffers()
{
...
void Exercise_Chapter9_2::BuildCrateGeometryBuffers()
{
