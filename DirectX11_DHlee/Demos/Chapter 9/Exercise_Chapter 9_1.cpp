...
class Exercise_Chapter9_1 : public D3DApp
{
public:
	Exercise_Chapter9_1(HINSTANCE hInstance);
	~Exercise_Chapter9_1();
	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise_Chapter9_1 theApp(hInstance);
...
Exercise_Chapter9_1::Exercise_Chapter9_1(HINSTANCE hInstance)
...
{
	mMainWndCaption = TEXT("연습문제 9장 1번");
	mEnable4xMsaa = false;
...
Exercise_Chapter9_1::~Exercise_Chapter9_1()
{
...
void Exercise_Chapter9_1::OnResize()
{
	D3DApp::OnResize();
...
void Exercise_Chapter9_1::UpdateScene(float dt)
{
...
void Exercise_Chapter9_1::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//연습문제 9_1
		//SampleMask는 멀티샘플 렌더타겟에서 어떤 샘플을 보여줄지를 선택하는 것
		//단일샘플 렌더 타겟에서는 마지막 mask인 1의자리 값에 따라 오브젝트가 표현되는지를 결정한다
...
void Exercise_Chapter9_1::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Exercise_Chapter9_1::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Exercise_Chapter9_1::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Exercise_Chapter9_1::GetHillHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Exercise_Chapter9_1::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void Exercise_Chapter9_1::BuildLandGeometryBuffers()
{
...
void Exercise_Chapter9_1::BuildWaveGeometryBuffers()
{
...
void Exercise_Chapter9_1::BuildCrateGeometryBuffers()
{
