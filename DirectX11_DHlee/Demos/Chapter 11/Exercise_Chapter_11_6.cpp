...
class Exercise_Chapter_11_6 : public D3DApp
{
public:
	Exercise_Chapter_11_6(HINSTANCE hInstance);
	~Exercise_Chapter_11_6();
	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise_Chapter_11_6 theApp(hInstance);
...
Exercise_Chapter_11_6::Exercise_Chapter_11_6(HINSTANCE hInstance)
...
{
	mMainWndCaption = TEXT("11장 연습문제 6~7번");
	mEnable4xMsaa = true;
...
Exercise_Chapter_11_6::~Exercise_Chapter_11_6()
{
...
bool Exercise_Chapter_11_6::Init()
{
	if (!D3DApp::Init())
		return false;
	...
	Effects::InitTreeFX(md3dDevice, TEXT("TreeSprite_Modified.fxo"));
...
void Exercise_Chapter_11_6::OnResize()
{
	D3DApp::OnResize();
...
void Exercise_Chapter_11_6::UpdateScene(float dt)
{
...
void Exercise_Chapter_11_6::DrawScene()
{
...
void Exercise_Chapter_11_6::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Exercise_Chapter_11_6::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Exercise_Chapter_11_6::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Exercise_Chapter_11_6::GetHillHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Exercise_Chapter_11_6::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void Exercise_Chapter_11_6::BuildLandGeometryBuffers()
{
...
void Exercise_Chapter_11_6::BuildWaveGeometryBuffers()
{
...
void Exercise_Chapter_11_6::BuildCrateGeometryBuffers()
{
...
void Exercise_Chapter_11_6::BuildTreeSpritesBuffer()
{
...
void Exercise_Chapter_11_6::DrawTreeSprites(CXMMATRIX viewProj)
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//연습문제 6
		// 0~15번으로 정점번호가 정해지고, Vertex Shader에 의해 2~17의 크기값이 결정됨
		md3dImmediateContext->DrawIndexed(TreeCount, 0, 0);
		//연습문제 7
		//Pirmitive ID가 15까지 존재함에도 불구하고 텍스쳐 배열에 저장된 최대 값으로 색인 한정이 일어남
		//md3dImmediateContext->Draw(TreeCount, 0);
		//4개씩 Draw하도록 선언하면 Primitive ID는 0~3이므로 색인 범위가 벗어나지 않는다.
