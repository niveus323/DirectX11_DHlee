...
{
	mMainWndCaption = TEXT("7장 조명 연습문제");
	...
	//연습문제 1번
	// Directional light.
	mDirLight.Ambient = XMFLOAT4(0.8f, 0.2f, 0.2f, 1.0f);
	mDirLight.Diffuse = XMFLOAT4(0.8f, 0.2f, 0.2f, 1.0f);
	mDirLight.Specular = XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
	mDirLight.Direction = XMFLOAT3(0.57735f, -0.57735f, 0.57735f);

	// Point light--position is changed every frame to animate in UpdateScene function.
	mPointLight.Ambient = XMFLOAT4(0.2f, 0.8f, 0.2f, 1.0f);
	...
	mSpotLight.Diffuse = XMFLOAT4(0.2f, 0.2f, 0.8f, 1.0f);
	...
	//연습문제 2번 - 반영광의 반영 지수는 w값에 저장된다
	//조명은 x,y,z만 사용하기 때문에 사용하지 않는 알파 성분을 반영 지수 값을 저장하는 용도로 사용
...
void LightingApp::UpdateScene(float dt)
{
	...
	if ((mTimer.GameTime() - t_base) >= 0.25f)
	{
		t_base += 0.25f;
	...
	mPointLight.Position.x = 70.0f * cosf(0.2f * mTimer.GameTime());
	mPointLight.Position.z = 70.0f * sinf(0.2f * mTimer.GameTime());
...
//연습문제 4번 - 반영광(Specular)의 각도는 Spot값으로 조절
LRESULT LightingApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		{
			mSpotLight.Spot = max(mSpotLight.Spot - 1.0f, 0.0f);
		}
		return 0;
	}
...
{
	//연습문제 3번
	std::ifstream fin("../Fx/CartoonRendering.fxo", std::ios::binary);
