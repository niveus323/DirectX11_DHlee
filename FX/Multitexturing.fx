...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	float4 tex1Color = float4(1, 1, 1, 1);
	float4 tex2Color = float4(1, 1, 1, 1);
	if (gUseTexure)
	{
		// Sample texture.
		tex1Color = gDiffuseMap[0].Sample(gSamplerState, pin.Tex);
		tex2Color = gDiffuseMap[1].Sample(gSamplerState, pin.Tex);
	}
	
	//
	// Lighting.
	//
	float4 blendColor = tex1Color * tex2Color;

	float4 litColor = blendColor;
	if (gLightCount > 0)
	{
		...
		litColor = blendColor * (ambient + diffuse) + spec;
	}
	
	// Common to take alpha from diffuse material and texture.
	litColor.a = gMaterial.Diffuse.a * blendColor.a;
