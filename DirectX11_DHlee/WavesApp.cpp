...
void WavesDemo::UpdateScene(float dt)
{
	...
	{
		v[i].Pos = mWaves[i];
		v[i].Color = Convert::ToXmFloat4(Colors::Blue);
	}
...
//동적 정점 버퍼를 사용하여 시간에 따라 정점 정보가 변하는 애니메이션을 표현
void WavesDemo::BuildWavesGeometryBuffers()
{
	...
	D3D11_BUFFER_DESC vbd{};
    vbd.Usage = D3D11_USAGE_DYNAMIC;	//동적 정점 버퍼임을 명시
	vbd.ByteWidth = sizeof(Vertex) * mWaves.VertexCount();
    vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;	
    vbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;	//CPU의 버퍼에 자료를 기록할 수 있도록 플래그 설정
    vbd.MiscFlags = 0;
