...
class Exercise_Chapter9_6 : public D3DApp
{
public:
	Exercise_Chapter9_6(HINSTANCE hInstance);
	~Exercise_Chapter9_6();
	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise_Chapter9_6 theApp(hInstance);
...
Exercise_Chapter9_6::Exercise_Chapter9_6(HINSTANCE hInstance)
	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f), mBlendState(0)
{
	mMainWndCaption = TEXT("연습문제 9장 6번");
	mEnable4xMsaa = false;
...
Exercise_Chapter9_6::~Exercise_Chapter9_6()
{
...
void Exercise_Chapter9_6::OnResize()
{
	D3DApp::OnResize();
...
void Exercise_Chapter9_6::UpdateScene(float dt)
{
	...
	//
	// 연습문제 6번
	// 혼합 상태의 RenderTargetWriteMask는 혼합 결과를 어떤 색상 채널에 기록할 것인지를 결정
	// RenderTargetWriteMask를 비활성화하면 픽셀 셰이더로부터 나온 색상이 어떤 마스크에도 적용되지 않음
	// 밝은 색상일수록 마스크된 색상이 표현되고 어두운 색상일수록 마스크되지 않은 색상이 표현된다
	//
	if (GetAsyncKeyState('1') & 0x8000)
	{
		D3D11_BLEND_DESC blendDesc = { 0 };
		mBlendState->GetDesc(&blendDesc);
		blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;
		HR(md3dDevice->CreateBlendState(&blendDesc, &mBlendState));
	}
	...
	if (GetAsyncKeyState('2') & 0x8000)
	{
		D3D11_BLEND_DESC blendDesc = { 0 };
		mBlendState->GetDesc(&blendDesc);
		blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_RED | D3D11_COLOR_WRITE_ENABLE_ALPHA;
		HR(md3dDevice->CreateBlendState(&blendDesc, &mBlendState));
	}
	...
	if (GetAsyncKeyState('3') & 0x8000)
	{
		D3D11_BLEND_DESC blendDesc = { 0 };
		mBlendState->GetDesc(&blendDesc);
		blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_GREEN | D3D11_COLOR_WRITE_ENABLE_ALPHA;
		HR(md3dDevice->CreateBlendState(&blendDesc, &mBlendState));
	}
	...
	if (GetAsyncKeyState('4') & 0x8000)
	{
		D3D11_BLEND_DESC blendDesc = { 0 };
		mBlendState->GetDesc(&blendDesc);
		blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_BLUE | D3D11_COLOR_WRITE_ENABLE_ALPHA;
		HR(md3dDevice->CreateBlendState(&blendDesc, &mBlendState));
	}
}

void Exercise_Chapter9_6::DrawScene()
{
...
void Exercise_Chapter9_6::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Exercise_Chapter9_6::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Exercise_Chapter9_6::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Exercise_Chapter9_6::GetHillHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Exercise_Chapter9_6::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void Exercise_Chapter9_6::BuildLandGeometryBuffers()
{
...
void Exercise_Chapter9_6::BuildWaveGeometryBuffers()
{
...
void Exercise_Chapter9_6::BuildCrateGeometryBuffers()
{
