...
class Exercise_Chapter10_7 : public D3DApp
{
public:
	Exercise_Chapter10_7(HINSTANCE hInstance);
	~Exercise_Chapter10_7();
...
private:
	...
	ID3D11Buffer* mBoltVB;
	ID3D11Buffer* mBoltIB;
	...
	UINT mBoltAnimIndex;
	float mBoltAnimFrame;
	UINT mBoltAnimMaxCount;
};

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance,
	PSTR cmdLine, int showCmd)
{
	Exercise_Chapter10_7 theApp(hInstance);
...
Exercise_Chapter10_7::Exercise_Chapter10_7(HINSTANCE hInstance)
	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f), mBoltAnimMaxCount(60), mBoltAnimFrame(0.0f), mBoltAnimIndex(0)
{
	mMainWndCaption = TEXT("연습문제 10장 7번");
	mEnable4xMsaa = false;
	...
	XMMATRIX boltOffset = XMMatrixTranslation(8.0f, 0.0f, -15.0f);
...
Exercise_Chapter10_7::~Exercise_Chapter10_7()
{
...
bool Exercise_Chapter10_7::Init()
{
	if (!D3DApp::Init())
		return false;
	...
	LoadTexture(TEXT("../Textures/BoltAnim"), TEXT("Bolt"), TEXT("bmp"), 60);

	BuildLandGeometryBuffers();
	BuildWaveGeometryBuffers();
	BuildCrateGeometryBuffers();
	BuildBoltGeometryBuffers();
...
void Exercise_Chapter10_7::OnResize()
{
	D3DApp::OnResize();
...
void Exercise_Chapter10_7::UpdateScene(float dt)
{
	...
	XMMATRIX boltTranslation = XMMatrixTranslation(mTimer.GameTime() * 0.02f, 0.0f, 0.0f);
	XMStoreFloat4x4(&mBoltTexTransform, boltScale * boltTranslation);

	UpdateAnimation(0.03333f);
}

void Exercise_Chapter10_7::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		md3dImmediateContext->IASetVertexBuffers(0, 1, &mBoltVB, &stride, &offset);
		md3dImmediateContext->IASetIndexBuffer(mBoltIB, DXGI_FORMAT_R32_UINT, 0);
		...
		Effects::BasicFX->SetDiffuseMap(mBoltSRV[mBoltAnimIndex]);
		Effects::BasicFX->SetSamplerstate(mSamplerState);
...
void Exercise_Chapter10_7::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Exercise_Chapter10_7::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Exercise_Chapter10_7::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Exercise_Chapter10_7::GetHillHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Exercise_Chapter10_7::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void Exercise_Chapter10_7::BuildLandGeometryBuffers()
{
...
void Exercise_Chapter10_7::BuildWaveGeometryBuffers()
{
...
void Exercise_Chapter10_7::BuildCrateGeometryBuffers()
{
...
void Exercise_Chapter10_7::BuildBoltGeometryBuffers()
{
	...
	HR(md3dDevice->CreateBuffer(&vbd, &vinitData, &mBoltVB));

	mBoltIndexCount = numSlices * 6;
	UINT indices[numSlices * 6]{}; // 6 indices per quad
	...
	D3D11_BUFFER_DESC ibd{};
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * mBoltIndexCount;
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA iinitData{};
	iinitData.pSysMem = indices;
	HR(md3dDevice->CreateBuffer(&ibd, &iinitData, &mBoltIB));
}

void Exercise_Chapter10_7::LoadTexture(wstring path, wstring name, wstring extention, int maxAnimFrame)
{
	ScratchImage boltImage;
	mBoltAnimMaxCount = maxAnimFrame;
	...
	for (int i = 1; i <= maxAnimFrame; i++)
	{
		...
		wstring fileStr = path + TEXT("/")  + name + numStr + TEXT(".") + extention;

		LoadFromWICFile(fileStr.c_str(), WIC_FLAGS_NONE, nullptr, boltImage);
		CreateShaderResourceView(md3dDevice, boltImage.GetImages(), boltImage.GetImageCount(), boltImage.GetMetadata(), &mBoltSRV[i-1]);
	}
}

void Exercise_Chapter10_7::UpdateAnimation(float interval)
{
	mBoltAnimFrame += mTimer.DeltaTime();

	if (mBoltAnimFrame >= interval)
	{
		mBoltAnimFrame = 0;
		++mBoltAnimIndex;
		if (mBoltAnimIndex == mBoltAnimMaxCount)
			mBoltAnimIndex = 0;
	}
